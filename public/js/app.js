...
function WaitingRoomScreen(store) {
  ...
  return {
    ...
    children: [
      { tag: 'h2', children: ['Waiting Room'] },
      { tag: 'div', children: [`Players: ${state.playerCount} / 4`] },
      {
        tag: 'ul',
      ...
      state.countdown
        ? { tag: 'div', children: [`Game starting in ${state.countdown} seconds`] }
        : null,
      ChatComponent(store)
    ].filter(Boolean)
  };
...
function GameScreen(store) {
  ...
  if (!window.gameControlsInitialized) {
    ...
    // Focus the game canvas when the game screen is shown
    setTimeout(() => {
  ...
  return {
    tag: 'div',
    attrs: { class: 'screen game-screen active' },
    children: [
      localPlayer ? PlayerHeaderComponent(localPlayer) : null,
      {
        ...
        attrs: {
          id: 'game-canvas',
          style: 'position: relative; width: 100%; height: 600px; background: #222;',
          tabindex: 0,
    ...
    children: [
      {
        ...
        children: (state.chatMessages || []).map(msg => ({
          tag: 'div',
          children: [`${msg.nickname}: ${msg.text}`]
        }))
      },
      {
        tag: 'input',
...
function TileComponent({ tileType, x, y }) {
  let backgroundImage = '';
  let zIndex = '1';
  ...
  return {
    ...
    attrs: {
      class: extraClass,
      style: `position: absolute; width: 32px; height: 32px; left: ${x * 32}px; top: ${y * 32}px; background-image: url(${backgroundImage}); background-size: cover; background-repeat: no-repeat; z-index: ${zIndex};`,
      'data-grid-x': x,
      'data-grid-y': y
    },
    children: []
...
function MapComponent(mapArray) {
  ...
  for (let y = 0; y < mapArray.length; y++) {
    for (let x = 0; x < mapArray[0].length; x++) {
      tiles.push(TileComponent({ tileType: mapArray[y][x], x, y }));
    }
  }
  return tiles;
}
...
function PlayerComponent(player) {
  ...
  return {
    ...
    attrs: {
      class: 'player',
      style: `position: absolute; width: 32px; height: 32px; overflow: hidden; z-index: 10; transition: transform 0.1s ease-out; transform: translate3d(${player.pixelX}px, ${player.pixelY}px, 2px); background-image: url(${spritePath}); background-size: cover; background-position: -${frameIndex * 32}px 0px;`
    },
    children: [
      {
        tag: 'div',
        attrs: {
          style: 'position: absolute; top: -20px; left: 50%; transform: translateX(-50%); font-size: 10px; color: #000; background-color: rgba(255,255,255,0.8); padding: 2px 4px; border-radius: 3px; white-space: nowrap;'
        },
        children: [player.nickname]
      }
    ]
...
function BombComponent(bomb) {
  return {
    ...
    attrs: {
      class: 'bomb',
      style: `position: absolute; width: 32px; height: 32px; background-image: url(/assets/bomb.png); background-size: cover; z-index: 5; transform: translate3d(${(bomb.pixelX ?? (bomb.gridX !== undefined ? bomb.gridX * 32 : 0))}px, ${(bomb.pixelY ?? (bomb.gridY !== undefined ? bomb.gridY * 32 : 0))}px, 1px);`
    },
    children: []
  };
}
...
function PowerUpComponent(powerUp) {
  return {
    ...
    attrs: {
      class: 'powerup',
      style: `position: absolute; width: 32px; height: 32px; background-image: url(/assets/${powerUp.type}.png); background-size: cover; z-index: 7; transform: translate3d(${powerUp.pixelX !== undefined ? powerUp.pixelX : powerUp.gridX * 32}px, ${powerUp.pixelY !== undefined ? powerUp.pixelY : powerUp.gridY * 32}px, 1px);`
    },
    children: []
  };
} 
...
function ExplosionComponent(explosion) {
  return {
    ...
    attrs: {
      class: 'explosion',
      style: `position: absolute; width: 32px; height: 32px; background-image: url(/assets/2.png); background-size: cover; z-index: 6; transform: translate3d(${(explosion.pixelX ?? (explosion.gridX !== undefined ? explosion.gridX * 32 : 0))}px, ${(explosion.pixelY ?? (explosion.gridY !== undefined ? explosion.gridY * 32 : 0))}px, 1px);`
    },
    children: []
  };
} 
